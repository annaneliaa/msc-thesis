#!/usr/bin/env bash
# Fills the 20 specific (family, grouping, scenario) cells the AIT-ADS
# baseline grid says are neither excluded nor leakage, just never run --
# see the "AIT-ADS Baseline Grid" artifact / MEMORY.md for the full 640-cell
...
#   classes under cscas_grouping):
#     ait_ads_zeroshot.py x 4 models x 2 scenarios                  (8)
#
# Every script is resumable (skips a combo whose result JSON already
# exists), so this is safe to re-run after an interruption, and safe to run
# alongside/after the general run_ait_ads_*.sh scripts without redoing their
# work -- AIT_ADS_SCENARIOS/AIT_ADS_GROUPING_METHODS below scope every step
...
# pulled (done automatically here via /api/pull), same requirement as
# run_ait_ads_zeroshot.sh.
#
