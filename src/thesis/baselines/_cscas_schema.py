...
from thesis.baselines._mining_modes import (  # noqa: F401 (re-exported)
    MINING_MODE_SUFFIX,
    MINING_MODES,
...
from thesis.baselines._mining_modes import active_mining_mode as _active_mining_mode
...
#: Attribute-mining tree mode for the three CSCAS mining baselines
#: (cscas_mining.py, cscas_mining_anomaly.py, cscas_mining_anomaly_iforest.py).
#: MINING_MODES/MINING_MODE_SUFFIX/mining_attribute_config[_anomaly]/
#: discard_attack_patterns now live in _mining_modes.py (imported above and
#: re-exported here unchanged) so the AIT-ADS mining baselines can share the
#: exact same definitions instead of a second copy. Selected via
#: CSCAS_MINING_MODE (env var, additive "_twotree" result-name suffix, same
#: pattern as CSCAS_SCHEMA), so single-tree result files are never
#: overwritten by a two-tree run.
...
def active_mining_mode() -> str:
    ...
    return _active_mining_mode("CSCAS_MINING_MODE")
