...
RandomForestClassifier for XGBClassifier, matching cscas_xgboost.py's
relationship to cscas_base.py. A straight swap of the RF pattern: XGBoost,
like RF, is tree-based, so no scaling is needed (unlike ait_ads_logreg.py).
...
def run_scenario(scenario: str, grouping_method: str) -> None:
    ...
    result_name = f"ait_ads_xgboost_{run_tag}"
    print(
    ...
    for condition, build_pool in pool_builders.items():
        print(f"\n=== {run_tag} / {condition} (XGBoost, base schema) ===")
    ...
    save_baseline_results(
        ...
        description=(
            ...
            "5-column base schema, XGBClassifier(n_estimators=100), random + "
