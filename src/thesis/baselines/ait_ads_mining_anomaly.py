...
exactly the same modes/config as cscas_mining_anomaly.py's own
CSCAS_MINING_MODE (see baselines/_mining_modes.py). "single_tree" (default)
mines with mining_attribute_config_anomaly("single_tree")'s defaults and
keeps the mined patterns as-is (both benign- and attack-leaning) -- the
original behavior, byte-reproducible with the existing
ait_ads_mining_anomaly*.json files. "two_tree" is an add-on, not a
replacement: it uses mining_attribute_config_anomaly("two_tree") and
additionally calls discard_attack_patterns() to drop every attack-leaning
mined pattern before the symbolic feature schema is built, so the anomaly
...
Run:
    ...
    python ait_ads_mining_anomaly.py                              # single-tree
    AIT_ADS_MINING_MODE=two_tree python ait_ads_mining_anomaly.py  # two-tree add-on
"""
...
# cscas_mining_anomaly.py's CSCAS_MINING_MODE. See _mining_modes.py.
...
def run_scenario(scenario: str, grouping_method: str) -> None:
    ...
    result_name = f"ait_ads_mining_anomaly_{run_tag}_ocsvm{MODE_SUFFIX}"
    print(
    ...
    # building the symbolic schema -- this detector's own training is
    # benign-only, so it should never be handed attack-derived features
    # either, even though mining itself needed both classes' labels to find
    # them. single_tree mode keeps mining_result as-is (byte-reproducible
    # with the existing ait_ads_mining_anomaly*.json files). See
    # _mining_modes.discard_attack_patterns.
    ...
    save_anomaly_results(
        ...
        description=(
            ...
            + "), OneClassSVM(kernel='rbf', nu=0.05) fit on benign-only "
